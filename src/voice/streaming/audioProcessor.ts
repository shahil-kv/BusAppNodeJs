...
export interface AudioProcessingOptions {
    ...
    encoding: 'pcm' | 'mulaw'; // Add encoding option
}
...
export class AudioProcessor {
    private static readonly DEFAULT_OPTIONS: AudioProcessingOptions = {
        ...
        encoding: 'mulaw' // Default to mulaw for Twilio
    };
     ...
     * μ-law is the standard encoding for telephony and Twilio Media Streams
     */
    private static pcmToMulaw(pcmBuffer: Buffer): Buffer {
        ...
        for (let i = 0; i < samples; i++) {
            ...
            mulawBuffer.writeUInt8(mulawSample, i);
        }

        logger.log(`[AudioProcessor] Converted ${samples} samples from PCM to μ-law`);
        return mulawBuffer;
    }
     ...
     */
    private static linearToMulaw(pcmSample: number): number {
        ...
        const quantization = (magnitude >> segment) & 0x0F;
        
        // Combine into μ-law byte
        const mulawByte = sign | ((7 - segment) << 4) | quantization;
        
        return mulawByte ^ 0xFF; // Invert all bits
    }

    /**
     * Apply encoding to processed audio
     */
    private static applyEncoding(buffer: Buffer, encoding: 'pcm' | 'mulaw'): Buffer {
        if (encoding === 'mulaw') {
            return this.pcmToMulaw(buffer);
        }
        return buffer; // Return as-is for PCM
    }
}
